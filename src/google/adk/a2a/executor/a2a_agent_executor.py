...
class A2aAgentExecutor(AgentExecutor):
  ...
  async def _handle_request(
    ...
    async for adk_event in runner.run_async(**run_args):
      task_result_aggregator.process_event(adk_event)
      for a2a_event in convert_event_to_a2a_events(
          adk_event, invocation_context
      ):
        event_queue.enqueue_event(a2a_event)
  ...
  async def _prepare_session(
    ...
    if not session_id:
      ...
      # override the non-existing context_id
      context.context_id = _to_a2a_context_id(
