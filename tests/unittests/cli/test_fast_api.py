...
def test_app(
  ...
  with (
    ...
    app = get_fast_api_app(
        ...
        memory_service_uri="",
        allow_origins=["*"],
        enable_a2a=False,  # Disable A2A for most tests
        host="127.0.0.1",
        port=8000,
    )
...
def test_app_with_a2a(
  ...
  with (
      ...
      patch("google.adk.cli.fast_api.InMemoryTaskStore") as mock_task_store,
      patch("google.adk.cli.fast_api.A2aAgentExecutor") as mock_executor,
      patch("google.adk.cli.fast_api.DefaultRequestHandler") as mock_handler,
      patch("google.adk.cli.fast_api.A2AStarletteApplication") as mock_a2a_app,
  ):
    # Configure mocks
...
def test_a2a_disabled_by_default(test_app):
  ...
  # The regular test_app fixture has enable_a2a=False
